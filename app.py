...
# =========================================================
# PREPARAÇÃO DOS DADOS A PARTIR DOS PARQUETS
# =========================================================
@st.cache_data
def load_data(ano):
    """
    ...
    # Renomeia POP_TOTAL para POPULACAO_TOTAL
...
# Definição das colunas utilizadas
COL_CODIGO_ROTA = 'COD_IBGE'
COL_MUNICIPIO = 'MUNICÍPIO'
COL_TIPO_COLETA = 'TIPO_COLETA_EXECUTADA'
COL_MASSA = 'MASSA_COLETADA'
COL_DESTINO = 'TIPO_DESTINO'
COL_UF = 'UF'
...
with tab_tradicional:
    ...
    # =========================================================
    # PANORAMA NACIONAL DE GERAÇÃO PER CAPITA E PARETO
    # =========================================================
    if municipio == municipios[0]:
        ...
        with st.spinner("Calculando estatísticas nacionais..."):
            df_massa_mun = df.groupby('MUNICÍPIO').agg({
                'MASSA_COLETADA': 'sum',
    ...
    if municipio == municipios[0]:
        ...
        with st.spinner("Consultando dados..."):
            mask_organicos = df[COL_TIPO_COLETA].astype(str).str.contains(
                "seletiva.*orgânico|orgânico.*seletiva", case=False, na=False, regex=True)
            df_org_ranking = df[mask_organicos].copy()
            ...
            if df_org_ranking.empty:
                ...
                for (mun, uf), grupo in ranking_data.groupby([COL_MUNICIPIO, "UF"]):
                    ...
                    receita_anual = 0.0
                    if massa_aterro_local > 0:
                        df_mun_caract = df[df[COL_MUNICIPIO] == mun]
                    ...
                    massa_total_municipio = df[df[COL_MUNICIPIO] == mun]['MASSA_COLETADA'].sum()
...
with tab_ia:
    ...
    amostras = df[COL_DESTINO].dropna().sample(min(20, len(df))).tolist()
    ...
    with st.spinner("🤖 Classificando todos os destinos com IA..."):
        df['destino_ia'] = classificar_todos_destinos(df, COL_DESTINO)
    ...
    if st.button("🔍 Executar Clusterização"):
        with st.spinner("Agrupando municípios por similaridade..."):
            try:
                ...
                X, df_cluster = preparar_dados_clusterizacao(df)
                if X.empty:
    ...
    opcoes_proj = ["BRASIL – Todos os municípios"] + sorted(df[COL_MUNICIPIO].unique())
    ...
    if municipio_proj:
        if municipio_proj == "BRASIL – Todos os municípios":
            ...
            df_mun_proj = df[df[COL_MUNICIPIO] == municipio_proj]
            massa_atual = df_mun_proj['MASSA_COLETADA'].sum()
            pop_atual = st.number_input(
    ...
    opcoes_sim = ["BRASIL – Todos os municípios"] + sorted(df[COL_MUNICIPIO].unique())
    ...
    if municipio_sim:
        if municipio_sim == "BRASIL – Todos os municípios":
            ...
            df_mun_sim = df[df[COL_MUNICIPIO] == municipio_sim]
    ...
    if df_org.empty:
        ...
        df_uf_mun = df[[COL_MUNICIPIO, "UF"]].drop_duplicates(subset=[COL_MUNICIPIO])
        ...
        df_total_mun = df.groupby(COL_MUNICIPIO).agg({COL_MASSA: 'sum'}).reset_index()
        ...
        df_uf_mun2 = df[[COL_MUNICIPIO, "UF"]].drop_duplicates(subset=[COL_MUNICIPIO])
        ...
        df_total_geral = df.groupby(COL_MUNICIPIO).agg({
            COL_MASSA: 'sum',
            "UF": 'first'
        }).reset_index()
    ...
    df_total = df.groupby(COL_MUNICIPIO).agg({
        COL_MASSA: 'sum',
        "UF": 'first'
    }).reset_index()
    ...
    df_seletiva = df[mask_organicos].groupby(COL_MUNICIPIO).agg({COL_MASSA: 'sum'}).reset_index()
...
with tab_diagnostico:
    ...
    **Use este diagnóstico para priorizar políticas públicas:** municípios com alta emissão e alta intensidade são os que mais se beneficiam com a implantação de compostagem ou melhoria da gestão de aterros.
    ...
    with st.spinner("⏳ Processando dados de todos os municípios..."):
        df_emissoes = calcular_emissoes_brutas_por_municipio(df)
    ...
    if df_emissoes.empty:
        ...
        # =========================================================
        # GRÁFICO DE CONCENTRAÇÃO DAS EMISSÕES (PARETO)
        # =========================================================
        st.markdown("---")
